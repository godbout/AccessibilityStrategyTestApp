...
// das uses FT.aSentence that is already heavily tested.
// see cas for more blah blah
class ASUT_NM_das_Tests: ASUT_NM_BaseTests {
    
    private func applyMoveBeingTested(on element: AccessibilityTextElement, _ vimEngineState: inout VimEngineState) -> AccessibilityTextElement {
        return asNormalMode.das(on: element, &vimEngineState) 
    }
