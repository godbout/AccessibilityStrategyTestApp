...
// cis uses FT.innerSentence that is already heavily tested.
// see cip for more blah blah
class ASUT_NM_cis_Tests: ASUT_NM_BaseTests {
    
    private func applyMoveBeingTested(on element: AccessibilityTextElement, _ vimEngineState: inout VimEngineState) -> AccessibilityTextElement {
        return asNormalMode.cis(on: element, &vimEngineState) 
    }
