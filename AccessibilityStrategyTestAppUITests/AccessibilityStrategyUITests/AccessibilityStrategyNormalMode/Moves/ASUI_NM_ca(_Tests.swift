...
class ASUI_NM_caLeftParenthesis_Tests: ASUI_NM_BaseTests {

    private func applyMoveBeingTested(appFamily: AppFamily) -> AccessibilityTextElement {
        var state = VimEngineState(appFamily: appFamily)
        
        return applyMove { asNormalMode.caLeftParenthesis(on: $0, &state) }
    }
...
// PGR and Electron
extension ASUI_NM_caLeftParenthesis_Tests {
