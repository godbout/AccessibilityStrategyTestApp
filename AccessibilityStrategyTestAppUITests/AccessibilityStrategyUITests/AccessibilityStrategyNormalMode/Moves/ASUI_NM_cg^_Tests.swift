...
class ASUI_NM_cgCaret_Tests: ASUI_NM_BaseTests {
    
    private func applyMoveBeingTested() -> AccessibilityTextElement {
        var vimEngineState = VimEngineState(appFamily: .pgR)
        
        return applyMove { asNormalMode.cgCaret(on: $0, &vimEngineState) }
    }
...
extension ASUI_NM_cgCaret_Tests {
    
    // NOTICE:
    // in macOS we can't add Blanks in front of a ScreenLine. when we try to do that
    // macOS adds the Blanks at the END OF THE PREVIOUS SCREENLINE. so here even tho
    // the move tested is cg^ it looks like a cg0 (ends at FirstNonBlank of ScreenLine)
...
"""
  ...
  C will now work with file lines     te from the caret ☀️ to before the linefeed
and of course this is in the case there is a linefeed at the end of the line.
"""
        )
