...
// see superman func for blah blah
class ASUI_NM_dgCaret_Tests: ASUI_NM_BaseTests {

    private func applyMoveBeingTested() -> AccessibilityTextElement {
        var vimEngineState = VimEngineState()
        
        return applyMove { asNormalMode.dgCaret(on: $0, &vimEngineState) }
    }
    
}


// test that we pass the right LineType parameter to superman func
extension ASUI_NM_dgCaret_Tests {
...
C will now work with file lines and is supposed to delete from the caret ☀️ to before the linefeed
   and of course this is       e there is a linefeed at the end of the line.
"""
        )
